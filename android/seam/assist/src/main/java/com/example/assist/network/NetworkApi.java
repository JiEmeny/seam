...
import com.example.assist.network.errorhandler.ExceptionHandle;
import com.example.assist.network.errorhandler.HttpErrorHandler;
import com.example.assist.network.interceptor.ResponseInterceptor;
...
public class NetworkApi {
    ...
    private static Retrofit getRetrofit(Class serviceClass) {
        ...
        //设置数据解析器，会自动把请求返回的结果（json字符串）通过Gson转化工厂自动转化成与其结果
    ...
    /**
     * 配置RxJava完成线程的切换，如果是Kotlin完全可以直接使用协程
     *
    ...
    protected static <T> Function<T, T> getAppErrorHandler() {
        return t -> {
            ...
            if (t instanceof BaseResponse && ((BaseResponse) t).responseCode >= 500) {
                //通过这个异常处理，得到用户可以知道的原因
                ExceptionHandle.ServerException exception = new ExceptionHandle.ServerException();
                exception.code = ((BaseResponse) t).responseCode;
                exception.message = ((BaseResponse) t).responseError != null ? ((BaseResponse) t).responseError : "";
                throw exception;
            }
            return t;
        };
